...
router = APIRouter()  # Correção: Adicionando os parênteses corretamente
...
historico_de_compras: Dict[int, List[int]] = {}  # Correção: Nome padronizado
...
def criar_produto(produto: CriarProduto) -> Produto:
    ...
    global contador_produto  # Correção: Nome ajustado para minúsculas

    novo_produto = Produto(id=contador_produto, **produto.model_dump())
    produtos.append(novo_produto)
    contador_produto += 1  # Correção: Nome ajustado para minúsculas
...
def adicionar_historico_compras(
    ...
    if usuario_id not in [usuario.id for usuario in usuarios]:  # Correção: Indentação corrigida
